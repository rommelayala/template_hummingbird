...
from lib.login.veepee_es_login import robust_home_login_accept_cookies
    ...
    if local_retry_count >= 0:
        try:
            with context.new_page() as new_page:
                # ----------------------------------------------------------------
                logging.debug("-------------test_travel_page_campaigns-------------")
                page, logging_in = robust_home_login_accept_cookies(new_page)
                # ----------------------------------------------------------------
                page.goto(variables.vp_sites_map["ES"]["vp_site_root_url"]
                          + variables.vp_sites_map["ES"]["vp_site_travel_top_menu"])
                # ----------------------------------------------------------------
                ...
                data_to_test_pictures = get_data_to_test_pictures(g_imgs, "ES")
                g_banners = page.query_selector_all('article')
                data_to_test_banners = get_data_to_test_home_banners(context, g_banners, "ES")
                # ------ Launching tests
                master_check_picture_images_in_articles(data_to_test_pictures, "ES", "travel_page")
                banners_to_retest = master_travel_check_campaign_link_is_not_dead_v2(context, data_to_test_banners, "ES")
                if len(banners_to_retest) > 0:
                    if local_retry_count >= 0:
                        logging.debug(f"Relaunching failed banner --------------------failed {len(banners_to_retest)}")
                        master_travel_check_campaign_link_is_not_dead_v2(context, banners_to_retest, "ES")
                    local_retry_count -= 1

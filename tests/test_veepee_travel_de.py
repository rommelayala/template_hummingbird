    ...
    if local_retry_count >= 0:
        try:
            with context.new_page() as new_page:
                ...
                # ----------------------------------------------------------------
                page.goto(variables.vp_sites_map["DE"]["vp_site_root_url"]
                          + variables.vp_sites_map["DE"]["vp_site_travel_top_menu"])
                # ----------------------------------------------------------------
                ...
                banners_to_retest = master_travel_check_campaign_link_is_not_dead_v2(context, data_to_test_banners, "DE")
                if len(banners_to_retest) > 0:
                    if local_retry_count >= 0:
                        logging.debug("Relaunching failed banner")
                        master_travel_check_campaign_link_is_not_dead_v2(context, banners_to_retest, "DE")
                    local_retry_count -= 1

                logging.debug("Launching check_picture_images_in_campaigns")
                master_check_picture_images_in_articles(data_to_test_pictures, "DE", "travel_page")
                # ------ Logging OUT
                logging_out(page)

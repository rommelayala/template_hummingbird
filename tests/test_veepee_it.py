...
from lib.login.veepee_it_login import robust_home_login_accept_cookies
    ...
    if local_retry_count >= 0:
        try:
            with context.new_page() as new_page:
                ...
                data_to_test_pictures = get_data_to_test_pictures(g_imgs, "IT")
                g_banners = page.query_selector_all('article')
                data_to_test_banners = get_data_to_test_home_banners(context, g_banners, "IT")

                # ------ Launching tests
                logging.debug("Launching master_home_page_banners_link_redirect_to_home_page_v2")
                banners_to_retest = master_home_page_banners_link_redirect_to_home_page_v2(context, data_to_test_banners, "IT")
                if len(banners_to_retest) > 0:
                    if local_retry_count >= 0:
                        logging.debug(f"Relaunching failed banner ---> {banners_to_retest}")
                        master_home_page_banners_link_redirect_to_home_page_v2(context, banners_to_retest, "IT")
                    local_retry_count -= 1

                logging.debug("Launching check_picture_images_in_campaigns")
                master_check_picture_images_in_articles(data_to_test_pictures, "IT", "home_page")
                # ------ Logging OUT
                logging_out(page)

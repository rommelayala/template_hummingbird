...
from lib.login.veepee_fr_login import robust_home_login_accept_cookies
...
def test_travel_page_campaigns(context: BrowserContext, local_retry_count=retry_count):
    if local_retry_count >= 0:
        try:
            url = variables.vp_sites_map["FR"]["vp_site_root_url"] + variables.vp_sites_map["FR"]["vp_site_travel_top_menu"]
            with context.new_page() as new_page:
                ...
                data_to_test_pictures = get_data_to_test_pictures(g_imgs, "FR")
                g_banners = page.query_selector_all('article')
                data_to_test_banners = get_data_to_test_home_banners(context, g_banners, "FR")

                logging.debug("All images collected")
                # ------ Launching tests
                #if logging_in:
                logging.debug("Launching master_travel_check_campaign_link_is_not_dead_v2")
                banners_to_retest = master_travel_check_campaign_link_is_not_dead_v2(context, data_to_test_banners, "FR")
                if len(banners_to_retest) > 0:
                    if local_retry_count >= 0:
                        logging.debug("Relaunching failed banner")
                        master_travel_check_campaign_link_is_not_dead_v2(context, banners_to_retest, "FR")
                    local_retry_count -= 1

                logging.debug("Launching check_picture_images_in_campaigns")
                master_check_picture_images_in_articles(data_to_test_pictures, "FR", "travel_page")
                # ------ Logging OUT
                logging_out(page)

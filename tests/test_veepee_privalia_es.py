...
def test_homepage_check_campaigns(context: BrowserContext):
    #vp_root_link = variables.vp_sites_map['PRIVALIA']['vp_site_root_url']
    page = context.new_page()
    page.goto(
        variables.vp_sites_map['PRIVALIA-ES']['vp_site_root_url']
    )
    page.get_by_role("button", name="Aceptar todas las cookies").click()
    # login
    page.get_by_role("button", name="Identificarme").click()
    page.get_by_role("button", name="Conectarme").nth(1).click()
    time.sleep(2)
    page.get_by_label("Email*").click()
    page.get_by_label("Email*").fill("automation.vp.es@gmail.com")
    page.get_by_label("Contraseña*").click()
    page.get_by_label("Contraseña*").fill("@Ut0mati0n.es")
    page.get_by_role("button", name="Conectarme").click()
    # End Login
    page.get_by_role("heading", name="DESTACADOS").click()
    page.keyboard.down('End')
    time.sleep(1)
    page.get_by_role("heading", name="HOY ESTRENAMOS").click()
    page.keyboard.down('End')
    time.sleep(1)
    page.get_by_role("heading", name="MÁS ARTÍCULOS").click()
    page.keyboard.down('End')
    time.sleep(1)
    page.get_by_role("heading", name="ÚLTIMO DÍA").click()
    page.keyboard.down('End')
    time.sleep(1)
    page.get_by_role("heading", name="PRÓXIMAMENTE").click()
    page.keyboard.down('End')
...
def check_campaign_link_is_not_dead(context, campaigns_catalogs):
    error = 0
    success = 0
    vp_root_link = variables.vp_sites_map['PRIVALIA-ES']['vp_site_root_url']

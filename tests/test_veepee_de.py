...
from lib.login.veepee_de_login import robust_home_login_accept_cookies
    ...
    if local_retry_count >= 0:
        try:
            with context.new_page() as new_page:
                ...
                data_to_test_pictures = get_data_to_test_pictures(g_imgs, "DE")

                g_banners = page.query_selector_all('article')
                data_to_test_banners = get_data_to_test_home_banners(context, g_banners, "DE")
                # ------ Launching tests
                logging.debug("Launching master_home_page_banners_link_redirect_to_home_page_v2")
                banners_to_retest = master_home_page_banners_link_redirect_to_home_page_v2(context, data_to_test_banners, "DE")
                if len(banners_to_retest) > 0:
                    if local_retry_count >= 0:
                        logging.debug(f"Relaunching failed banner -----{local_retry_count} / {variables.retry}")
                        master_home_page_banners_link_redirect_to_home_page_v2(context, banners_to_retest, "DE")
                    local_retry_count -= 1

                logging.debug("Launching check_picture_images_in_campaigns")
                master_check_picture_images_in_articles(data_to_test_pictures, "DE", "home_page")
                # ------ Logging OUT
                logging_out(page)

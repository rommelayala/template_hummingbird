    ...
    if local_retry_count >= 0:
        try:
            with context.new_page() as new_page:
                ...
                time.sleep(2)
                # ----------------------------------------------------------------
                url = f"{variables.vp_sites_map['LU']['vp_site_root_url']}{variables.vp_sites_map['LU']['vp_site_travel_top_menu']}"
                page.goto(url)
                # ----------------------------------------------------------------
                ...
                banners_to_retest = master_travel_check_campaign_link_is_not_dead_v2(context, data_to_test_banners, "LU")
                if len(banners_to_retest) > 0:
                    if local_retry_count >= 0:
                        logging.debug("Relaunching failed banner")
                        master_travel_check_campaign_link_is_not_dead_v2(context, banners_to_retest, "LU")
                    local_retry_count -= 1
                # ------ Launching tests
                master_check_picture_images_in_articles(data_to_test_pictures, "LU", "travel_page")
                # ------ Logging OUT
                logging_out(page)

    ...
    if local_retry_count >= 0:
        try:
            with context.new_page() as new_page:
                ...
                # ----------------------------------------------------------------
                url = f"{variables.vp_sites_map['AT']['vp_site_root_url']}{variables.vp_sites_map['AT']['vp_site_travel_top_menu']}"
                page.goto(url)
                # ----------------------------------------------------------------
                ...
                logging.debug("All images collected in travel page")
                data_to_test_pictures = get_data_to_test_pictures(g_imgs, "AT")
                logging.debug("Preparing images to test")
                # ------ Launching tests
                #if logging_in:
                logging.debug("Launching master_travel_check_campaign_link_is_not_dead_v2")
                banners_to_retest = master_travel_check_campaign_link_is_not_dead_v2(context, data_to_test_banners, "AT")
                if len(banners_to_retest) > 0:
                    if local_retry_count >= 0:
                        logging.debug("Relaunching failed banner")
                        master_travel_check_campaign_link_is_not_dead_v2(context, banners_to_retest, "AT")
                    local_retry_count -= 1

                master_check_picture_images_in_articles(data_to_test_pictures, "AT", "travel_page")
                # ------ Logging OUT
                logging_out(page)

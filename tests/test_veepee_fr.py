    ...
    if local_retry_count >= 0:
        try:
            with context.new_page() as new_page:
                ...
                # ------ Launching tests
                banners_to_retest = master_home_page_banners_link_redirect_to_home_page_v2(context, data_to_test_banners, "FR")
                if len(banners_to_retest) > 0:
                    if local_retry_count >= 0:
                        logging.debug("Relaunching failed banner")
                        master_home_page_banners_link_redirect_to_home_page_v2(context, banners_to_retest, "FR")
                    local_retry_count -= 1

                master_check_picture_images_in_articles(data_to_test_pictures, "FR", "home_page")
                # ------ Logging OUT
                logging_out(page)

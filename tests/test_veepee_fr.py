...
def test_homepage_check_campaigns_fr(context: BrowserContext):
    ...
    # page.query_selector_all("div>a[href^='https://www.veepee.fr/gr/catalog']"),
    ...
    # ------ Launching tests
    logging.debug("Launching check_campaign_link_is_not_dead_fr")
    check_campaign_link_is_not_dead_fr(context, campaigns_catalogs)
    logging.debug("Launching check_picture_images_in_campaigns_fr")
    # todo: comment/uncomment to work
    check_picture_images_in_campaigns_fr(context, pictures)


def check_campaign_link_is_not_dead_fr(context, campaigns_catalogs):
    error = 0
    success = 0
    vp_root_link = variables.vp_sites_map['FR']['vp_site_root_url']
    ...
    for campaigns_catalog in campaigns_catalogs:
        for campaign in campaigns_catalog:
            try:
                ...
                if (
                        href_url == "" or check_if_url_in_exceptions(href_url)
                        # or "/web/catalog/v1/sale/" in href_url
                        # or "bit.ly" in href_url
                        # or "club.veepee.fr" in href_url
                    ...
                    # check web element that is skipped
                    # logging.debug(f'SKIPPING Url {href_url} in component {web_element_a.inner_html()}')

                else:
                    # WARNING you MUST USE "stop_and_save_trace(context, page_camp, timestamp)" to complete trace processing
                    start_save_trace(context)
                ...
                logging.exception(e)
                # WARNING you MUST USE start_save_trace(context)
                # Stop tracing and export it into a zip archive.
                stop_and_save_trace(context, page_camp, timestamp)
                error += 1
    ...
    print_report_execution_error_success_in_console("check_campaign_link_is_not_dead_fr", start_time_before_all_checks,
                                                    error,
                                                    success)
...
def stop_and_save_trace(context, page_camp, timestamp):
    # Stop tracing and export it into a zip archive.
...
def check_picture_images_in_campaigns_fr(context, pictures):
    error = 0
    success = 0
        ...
        for source in sources:
            try:
                ...
                data = {
                    ...
                    "tags": ["Failed", "check_picture_images_in_campaigns_fr"]
                }
                # Inject into Elastic
    ...
    print_report_execution_error_success_in_console("check_picture_images_in_campaigns_fr", start_time, error, success)
...
def run(playwright: Playwright) -> None:
    ...
    with browser.new_context() as context:
        # ---------------------
        test_homepage_check_campaigns_fr(context=context)
        # ---------------------

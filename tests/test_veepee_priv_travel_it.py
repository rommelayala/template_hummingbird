...
from lib.login.veepee_priv_it_login import robust_home_login_accept_cookies
    ...
    if local_retry_count >= 0:
        try:
            with context.new_page() as new_page:
                ...
                time.sleep(2)
                # ----------------------------------------------------------------
                url = f"{variables.vp_sites_map['PRIVALIA-IT']['vp_site_root_url']}{variables.vp_sites_map['PRIVALIA-IT']['vp_site_travel_top_menu']}"
                page.goto(url)
                # ----------------------------------------------------------------
                ...
                data_to_test_pictures = get_data_to_test_pictures(g_imgs, "PRIVALIA-IT")

                g_banners = page.query_selector_all('article')
                data_to_test_banners = get_data_to_test_home_banners(context, g_banners, "PRIVALIA-IT")

                # ------ Launching tests
                banners_to_retest = master_travel_check_campaign_link_is_not_dead_v2(context, data_to_test_banners, "PRIVALIA-IT")
                if len(banners_to_retest) > 0:
                    if local_retry_count >= 0:
                        logging.debug("Relaunching failed banner")
                        master_travel_check_campaign_link_is_not_dead_v2(context, banners_to_retest, "PRIVALIA-IT")
                    local_retry_count -= 1

                master_check_picture_images_in_articles(data_to_test_pictures, "PRIVALIA-IT", "travel_page")
                # ------ Logging OUT
                logging_out(page)

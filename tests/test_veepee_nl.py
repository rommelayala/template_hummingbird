    ...
    if local_retry_count >= 0:
        try:
            with context.new_page() as new_page:
                ...
                page = scrolling_down_the_page(page, 1.5)
                # ----------------------------------------------------------------
                ...
                banners_to_retest = master_home_page_banners_link_redirect_to_home_page_v2(context, data_to_test_banners, "NL")
                if len(banners_to_retest) > 0:
                    if local_retry_count >= 0:
                        logging.debug("Relaunching failed banner")
                        master_home_page_banners_link_redirect_to_home_page_v2(context, banners_to_retest, "NL")
                    local_retry_count -= 1

                logging.debug("Launching check_picture_images_in_campaigns")
                master_check_picture_images_in_articles(data_to_test_pictures, "NL")
                # ------ Logging OUT
                logging_out(page)
            ...
            data = {
                ...
                "fail_level": "high",
                "timestamp": timestamp,
                "country": variables.vp_sites_map['NL']['country'],

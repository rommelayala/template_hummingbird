...
from lib.login.veepee_be_login import robust_home_login_accept_cookies
    ...
    if local_retry_count >= 0:
        try:
            with context.new_page() as new_page:
                page, logged_in = robust_home_login_accept_cookies(new_page)
                ...
                # ----------------------------------------------------------------
                logging.debug("Scrolling page to get all banners")
                page = scrolling_down_the_page(page, 1)
                # ----------------------------------------------------------------
                logging.debug("Getting all Webelements banners")
                g_imgs = page.locator("img[alt='banner']").all()
                data_to_test_pictures = get_data_to_test_pictures(g_imgs, "BE")

                g_banners = page.query_selector_all('article')
                data_to_test_banners = get_data_to_test_home_banners(context, g_banners, "BE")
                # ------ Launching tests
                logging.debug("Launching master_home_page_banners_link_redirect_to_home_page_v2")
                banners_to_retest = master_home_page_banners_link_redirect_to_home_page_v2(context, data_to_test_banners, "BE")
                if len(banners_to_retest) > 0:
                    logging.debug("Relaunching failed banner")
                    master_home_page_banners_link_redirect_to_home_page_v2(context, banners_to_retest, "BE")

                logging.debug("Launching check_picture_images_in_campaigns")
                master_check_picture_images_in_articles(data_to_test_pictures, "BE")
                # ------ Logging OUT
                logging_out(page)
            ...
            data = {
                ...
                "fail_level": "high",
                "timestamp": timestamp,
                "country": variables.vp_sites_map['BE']['country'],

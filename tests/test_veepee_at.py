...
def test_homepage_check_campaigns(context: BrowserContext):
    ...
    logging.debug(f"# Going to {variables.vp_sites_map['AT']['vp_site_root_url']}")
    page.goto(
        variables.vp_sites_map['AT']['vp_site_root_url']
    )
    logging.debug("Authorise cookies")
    time.sleep(1)
    page.locator("button[id^='onetrust-accept']").click()

    page.get_by_test_id("header-right-wrapper").get_by_role("button", name="Anmelden").click()
    page.get_by_role("button", name="Einloggen").nth(1).click()
    page.get_by_label("Email*").click()
    page.get_by_label("Email*").fill("automation.vp.at@mail.com")
    page.get_by_label("Passwort*").click()
    page.get_by_label("Passwort*").fill("@Ut0mati0n.es")
...
def check_campaign_link_is_not_dead(context, campaigns_catalogs):
    error = 0
    success = 0
    vp_root_link = variables.vp_sites_map['AT']['vp_site_root_url']
        ...
        for campaign in campaigns_catalog:
            try:
                ...
                data = {
                    ...
                    "fail_level": "high",
                    "timestamp": timestamp,
                    "country": variables.vp_sites_map['AT']['country'],
